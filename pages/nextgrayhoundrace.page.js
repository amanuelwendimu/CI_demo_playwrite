...
exports.NextgrayhoundracePage = class NextgrayhoundracePage {
  constructor(page) {
    this.page = page;
    
    this.nextgrayhoundraceLink = page.getByRole('link', { name: 'Next greyhound Race' });
    ...
    this.Nomarket= page.locator("//span[normalize-space()='No Markets Found']");
    this.netxtgrayhoundraceBackOddButton = page.locator("tbody tr:nth-child(1) td:nth-child(4) a:nth-child(1) span:nth-child(2)");
    this.netxtgrayhoundraceLayOddButton = page.locator("body > app-root:nth-child(1) > app-pages:nth-child(3) > div:nth-child(1) > div:nth-child(2) > app-main:nth-child(2) > mat-drawer-container:nth-child(2) > mat-drawer-content:nth-child(4) > app-content:nth-child(2) > app-market-layout:nth-child(2) > div:nth-child(1) > div:nth-child(3) > mat-accordion:nth-child(1) > app-market:nth-child(1) > mat-expansion-panel:nth-child(1) > div:nth-child(2) > div:nth-child(1) > div:nth-child(1) > div:nth-child(1) > div:nth-child(2) > app-market-runners:nth-child(1) > div:nth-child(1) > div:nth-child(1) > table:nth-child(1) > tbody:nth-child(2) > tr:nth-child(1) > td:nth-child(5) > a:nth-child(1) > span:nth-child(2)");
  ...
  async navigateTograyhoundeBet() {
    await this.nextgrayhoundraceLink.click();
...
async getEventName() {
  ...
  //   if (await this.Nomarket.isVisible()) return"No Markets Found ";
  //   return "Open";
  // }
  ...
  const backOddVisible = await this.netxtgrayhoundraceBackOddButton.isVisible().catch(() => false);
  const layOddVisible = await this.netxtgrayhoundraceLayOddButton.isVisible().catch(() => false);
  if (!backOddVisible && !layOddVisible) {
    return "No Odds";
  }
  return "Open";
}
  async placenextgrayhoundBackBet(stake) {
    const marketState = await this.isMarketAvailable();
    if (marketState !== "Open") {
      this.betPlaced = false;
      return { message: `Gray hound Back bet not placed. Market is ${marketState}` };
    }
    await this.netxtgrayhoundraceBackOddButton.click();
    ...
    if (await this.errorMessage.isVisible()) {
      ...
      this.betPlaced = false;
      return { message: `Gray hound Back bet not placed: ${msg}` };
    }
    await expect(this.placeBetButton).toBeEnabled({ timeout: 5000 });
    await this.placeBetButton.click();
    this.betPlaced = true;
    return { message: "Gray hound Back bet placed successfully", odd, stakeVal, profit };
  }
  
  async placeUnmatchedNextgrayhoundBackBet(stake, backodd) {
    const marketState = await this.isMarketAvailable();
    if (marketState !== "Open") {
      this.betPlaced = false;
      return { message: `Unmatched grayhound Back bet not placed. Market is ${marketState}` };
    }
    await this.netxtgrayhoundraceBackOddButton.click();
    await this.odd.fill('');
    ...
    if (await this.errorMessage.isVisible()) {
      const msg = (await this.errorMessage.textContent())?.trim();
      this.betPlaced = false;
      return { message: `Unmatched Gray hound race Back bet not placed: ${msg}` };
    }
    await expect(this.placeBetButton).toBeEnabled({ timeout: 5000 });
    await this.placeBetButton.click();

    this.betPlaced = true;
    return { message: "Unmatched Grayhound race Back bet placed successfully", backodd, stakeVal, profit };
  }
