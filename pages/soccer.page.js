...
exports.SoccerPage = class SoccerPage {
  constructor(page) {
    this.page = page;
    this.soccerLink = page.locator('app-header').getByRole('link', { name: 'Soccer' });
    ...
    this.soccerBackOddButton = page.locator("body > app-root:nth-child(1) > app-pages:nth-child(3) > div:nth-child(1) > div:nth-child(2) > app-main:nth-child(2) > mat-drawer-container:nth-child(2) > mat-drawer-content:nth-child(4) > app-content:nth-child(2) > app-market-layout:nth-child(2) > div:nth-child(1) > div:nth-child(3) > mat-accordion:nth-child(1) > app-market:nth-child(1) > mat-expansion-panel:nth-child(1) > div:nth-child(2) > div:nth-child(1) > div:nth-child(1) > div:nth-child(1) > div:nth-child(2) > app-market-runners:nth-child(1) > div:nth-child(1) > div:nth-child(1) > table:nth-child(1) > tbody:nth-child(2) > tr:nth-child(1) > td:nth-child(4) > a:nth-child(1) > span:nth-child(1)");
    this.soccerLayOddButton = page.locator("body > app-root:nth-child(1) > app-pages:nth-child(3) > div:nth-child(1) > div:nth-child(2) > app-main:nth-child(2) > mat-drawer-container:nth-child(2) > mat-drawer-content:nth-child(4) > app-content:nth-child(2) > app-market-layout:nth-child(2) > div:nth-child(1) > div:nth-child(3) > mat-accordion:nth-child(1) > app-market:nth-child(1) > mat-expansion-panel:nth-child(1) > div:nth-child(2) > div:nth-child(1) > div:nth-child(1) > div:nth-child(1) > div:nth-child(2) > app-market-runners:nth-child(1) > div:nth-child(1) > div:nth-child(1) > table:nth-child(1) > tbody:nth-child(2) > tr:nth-child(1) > td:nth-child(5) > a:nth-child(1) > span:nth-child(2)");
  ...
  async navigateTosoccerBet() {
    await this.soccerLink.click();
...
async getEventName() {
  ...
  if (await this.suspended.isVisible()) return "Suspended";
  if (await this.closed.isVisible()) return "Closed";
  //if (await this.Noodds.isVisible()) return"No Markets Found ";
  // ✅ Check if odds are missing even though market is open
  const backOddVisible = await this.soccerBackOddButton.isVisible().catch(() => false);
  const layOddVisible = await this.soccerLayOddButton.isVisible().catch(() => false);
  if (!backOddVisible && !layOddVisible) {
    return "No Odds";
  }
  return "Open";
}
  async placeSoccerBackBet(stake) {
    const marketState = await this.isMarketAvailable();
    if (marketState !== "Open") {
      this.betPlaced = false;
      return { message: `Soccer Back bet not placed. Market is ${marketState}` };
    }
    await this.soccerBackOddButton.click();
    ...
    if (await this.errorMessage.isVisible()) {
      ...
      this.betPlaced = false;
      return { message: `Soccer Back bet not placed: ${msg}` };
    }
    await expect(this.placeBetButton).toBeEnabled({ timeout: 5000 });
    await this.placeBetButton.click();
    this.betPlaced = true;
    return { message: "Soccer Back bet placed successfully", odd, stakeVal, profit };
  }
  async placeSoccerLayBet(stake) {
    const marketState = await this.isMarketAvailable();
    if (marketState !== "Open") {
      this.betPlaced = false;
      return { message: `Tennis Lay bet not placed. Market is ${marketState}` };
    }
    await this.soccerLayOddButton.click();
    await this.stakeInput.fill('');
    await this.stakeInput.type(stake.toString());
    const odd = await this.getOddValue();
    const stakeVal = parseFloat(stake);
    ...
    if (await this.errorMessage.isVisible()) {
      const msg = (await this.errorMessage.textContent())?.trim();
      this.betPlaced = false;
      return { message: `soccer Lay bet not placed: ${msg}` };
    }
    await expect(this.placeBetButton).toBeEnabled({ timeout: 5000 });
    await this.placeBetButton.click();
    this.betPlaced = true;
    return { message: "Soccer Lay bet placed successfully", odd, stakeVal, liability };
  }

  // ✅ Unmatched back bet
  async placeUnmatchedSoccerBackBet(stake, backodd) {
    const marketState = await this.isMarketAvailable();
    if (marketState !== "Open") {
      this.betPlaced = false;
    ...
    this.betPlaced = true;
    return { message: "Unmatched Soccer Back bet placed successfully", backodd, stakeVal, profit };
  }
  async placeUnmatchedSoccerLayBet(stake, layodd) {
    const marketState = await this.isMarketAvailable();
    if (marketState !== "Open") {
      this.betPlaced = false;
      return { message: `Unmatched Soccer Lay bet not placed. Market is ${marketState}` };
    }
    await this.soccerLayOddButton.click();
    ...
    if (await this.errorMessage.isVisible()) {
      const msg = (await this.errorMessage.textContent())?.trim();
      this.betPlaced = false;
      return { message: `Unmatched soccer Lay bet not placed: ${msg}` };
    }
    await expect(this.placeBetButton).toBeEnabled({ timeout: 5000 });
    await this.placeBetButton.click();
    this.betPlaced = true;
    return { message: "Unmatched soccer Lay bet placed successfully", layodd, stakeVal, liability };
  }
