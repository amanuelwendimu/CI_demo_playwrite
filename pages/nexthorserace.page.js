...
exports.NextHorseracePage = class NextHorseracePage {
  constructor(page) {
    this.page = page;
    this.nexthorseraceLink = page.getByRole('link', { name: 'Next Horse Race' });
  ...
  async navigateTohorseBet() {
    await this.nexthorseraceLink.click();
...
async getEventName() {
  ...
  const backOddVisible = await this.netxthorseraceBackOddButton.isVisible().catch(() => false);
  const layOddVisible = await this.netxthorseraceLayOddButton.isVisible().catch(() => false);
  ...
  async placenextHorseraceBackBet(stake) {
    const marketState = await this.isMarketAvailable();
    if (marketState !== "Open") {
      this.betPlaced = false;
      return { message: `Horse Race Back bet not placed. Market is ${marketState}` };
    }
    await this.netxthorseraceBackOddButton.click();
    ...
    if (await this.errorMessage.isVisible()) {
      ...
      this.betPlaced = false;
      return { message: `Horse race Back bet not placed: ${msg}` };
    }
    await expect(this.placeBetButton).toBeEnabled({ timeout: 5000 });
    await this.placeBetButton.click();
    this.betPlaced = true;
    return { message: "Horse race Back bet placed successfully", odd, stakeVal, profit };
  }
  
  async placeUnmatchedNextHorseraceBackBet(stake, backodd) {
    const marketState = await this.isMarketAvailable();
    if (marketState !== "Open") {
      this.betPlaced = false;
      return { message: `Unmatched Horse race Back bet not placed. Market is ${marketState}` };
    }
    await this.netxthorseraceBackOddButton.click();
    await this.odd.fill('');
    ...
    if (await this.errorMessage.isVisible()) {
      const msg = (await this.errorMessage.textContent())?.trim();
      this.betPlaced = false;
      return { message: `Unmatched Horse race Back bet not placed: ${msg}` };
    }
    await expect(this.placeBetButton).toBeEnabled({ timeout: 5000 });
    await this.placeBetButton.click();

    this.betPlaced = true;
    return { message: "Unmatched Horse race Back bet placed successfully", backodd, stakeVal, profit };
  }
