...
test.describe.serial("Next Gray hound Race Bets Flow", () => {
  ...
  test("Place Unmatched Grayhound Race Back Bets", async () => {
    for (const { stake, backodd } of testData) {
      const stakeVal = Number(stake);
      const oddVal = Number(backodd);
      const result = await grayHoundRacePage.placeUnmatchedNextgrayhoundBackBet(stakeVal, oddVal);
